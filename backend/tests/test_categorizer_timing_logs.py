...
LOGGER = "app.services.categorizer"
...
async def test_openai_success_pads_and_logs(monkeypatch, caplog):
    ...
    monkeypatch.setattr("app.services.categorizer.AsyncOpenAI", _fake_openai(create))
    ...
    assert any(
        re.match(
            r"^event=llm_categorize provider=openai model=gpt-test rows=2 batches=1 uncategorized=1 ms=\d+\.\d$", m
        )
        for m in msgs
    )
...
async def test_anthropic_failure_logs(monkeypatch, caplog):
    ...
    monkeypatch.setattr("app.services.categorizer.AsyncAnthropic", _fake_anthropic(create))
    await categorizer.categorize_transactions(_txns())
