...
def detect_bank_format(content: bytes) -> str:
    ...
    # Bank Leumi's literal first line is blank (commas only); the "Bank Leumi"
    # marker appears a couple of lines down, so scan the first several lines
    # rather than relying on the single-first-line check used below.
    for line in text.split("\n")[:15]:
        if "Bank Leumi" in line:
            return "leumi"
