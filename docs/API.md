...
served at `/api/docs` (Swagger UI), `/api/redoc` (ReDoc) and `/api/openapi.json` (raw schema) whenever
the backend is running; these sit under the `/api` prefix deliberately so the production nginx proxy in
`frontend/nginx.conf`, which only forwards `location /api/` to the backend, serves them on the deployed
site.
